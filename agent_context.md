...
## 2k. Accounting review of every figure (2026-08-10)

User asked whether the numbers make sense and whether there are accounting errors. Reviewed the app's figures against the source ledger (`CS_Ledger__Updated_June72026.xlsx`) and all three BCBS exports. Commit `59b3d3c`.

**Method check first**: re-derived BCBS's lifetime P&L from the raw GL by summing the 9 income/expense accounts — reproduces their official statement to the cent (1,590,406.98 income / 1,254,638.41 expense / 335,768.57 net), which validates the parsing approach. The `BCBS_ACCRUAL_2025` constants in `dashboard.ts` also reproduce exactly.

**What reconciles**: internal income 615,113.00 matches the spreadsheet exactly; the 8,183.44 expense difference reconciles category-by-category to post-June-7 additions with no double-counting. The Tuition netting matches all four spreadsheet figures (270,340 actual / −14,200 projected / 256,140 total / 3,076 variance). All group subtotals sum correctly. The 3990/3991 offsetting pair nets to zero *within each individual year*, so it can't distort a single-period window. No negative/zero amounts, no direction-vs-type mismatches.

**THE BIG ERROR — mine, from §2i.** The Reconciliation page showed "$90,453.73 Money in the Bank (BCBS balance sheet)". That is not cash. The lifetime P&L's three "Balance Sheet" lines sum to **exactly** the lifetime Net Income (90,453.73 + 30,305.00 + 215,009.84 = 335,768.57) because they **partition accumulated net income by fund class**. BCBS's books contain no cash-in-bank figure scoped to this program. I asserted otherwise to the user in §2i — corrected now. Replaced with the one genuinely comparable pair: our position at 2026-04-30 (where the GL ends) vs BCBS's closing balance on `2827 - Temporarily Restricted Fund: Contemplative Semester` = 192,567.71, a gap of ~55,283. **Trap for future readers**: our *current* figure (192,644.22 at Jun 25) sits within $77 of that 192,567.71. It is a coincidence, not a tie-out — at the matching date our figure is 247,850.83. Do not present that near-match as agreement.

**Fiscal-year check on the spliced accrual figure**: pressure-tested whether the July 22 PDF's "2026" column is calendar or fiscal. A Nov-1 fiscal year would give 747,137 for a *shorter* window than the PDF's 700,860 — impossible — so it's calendar 2026 and there's no Nov–Dec 2025 overlap with the GL-derived half. The two halves also reconcile account-by-account (New Course Income 477,400 and Scholarships 240,260 match to the cent; the residual is the 50,311 Hemera Grant, which has no GL account because it arrived after the Apr 30 cutoff). The splice holds, but recompute from one source if BCBS sends a fuller GL.
...
## 2r. Wider BCBS exports: 2023-2026 expense + full campaign income (2026-09-15)

Two files from Melissa, both much wider than anything before.

**`Barre Center...Account Transactions_6.xlsx`** — still account `4355` only, but now **2023-12-07 → 2026-08-17**, 496 rows, with a new *Related account* column. Adds the **first cohort (2024: 250 rows, 416,913.37)** and 2023 (2 rows, 1,175.00). 2025 is **179,251.90**, identical to the previous export. 2026 is **348,033.54**, up 4,000.00 on the 7/31 figure — exactly one new row: **2026-08-17 Earthdance Creative Living, 4,000.00, "Deposit for Earthdance Rental Agreement"**. New venue, presumably a future cohort, so do NOT fold it into 2025-26 program cost.

First sight of **credits against 4355** (11,298.25, all 2024): 1,000.01 Potash Hill deposit returned, six small Marty's Local grocery credits, and a **10,000.00 "adjustment to actual for Audit FY24"** dated 2024-12-31. Every prior export was debit-only.

**`Contemplative Semester Campaigns-2026-08-26.xlsx`** — generated by Melissa (not Erin). Salesforce campaigns, `Status = Paid`, `Campaign Name contains Contemplative Semester,CS`, 1/1/2023–12/31/2026. **252 rows, 1,146,116.28**; my parse reproduces the report's own stated totals exactly (including donor cover 3,965.96).

It is a **strict superset of Erin's GT report**: 2025 = 110 rows / 421,332.07 and 2026 = 31 rows / 193,882.76, matching `bcbs_gift_transactions` to the cent. It adds 111 rows for 2023-2024 plus four columns Erin lacks (parent campaign, campaign, transaction type, payment method, registration amount) — but **drops account name and email**, which only Erin's report carries. The two sources are complementary; do not replace one with the other.

| Year | Rows | Current amount |
|---|---|---|
| 2023 | 2 | 106,400.00 |
| 2024 | 109 | 424,501.45 |
| 2025 | 110 | 421,332.07 |
...
Three campaigns, all under parent "Restricted": *Contemplative Semester* 1,071,813.19, *Contemplative Semester College Accreditation* 66,680.12 (the CEU fees), *Contemplative Semester Cancellations* 7,622.97 (two 2024 rows). By type: Donation 548,268.72, Class Registration 523,544.47, CEU Fee 66,680.12, Cancellation Fee 7,622.97.

**CEU / college-accreditation income by year: 2024 24,680.12, 2025 20,000.00, 2026 22,000.00.** The 2024 figure is where the Program Costs tab's 24,680 "Naropa Accreditation" budget line came from, and 20,000.00 independently confirms `BCBS_2025.collegeAccreditation`.
